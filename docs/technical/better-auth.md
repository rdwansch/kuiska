...
- [Comparison](/llms.txt/docs/comparison.md): Comparison of Better Auth versus other auth libraries and services.
...
- [CLI](/llms.txt/docs/concepts/cli.md): Learn about the Better Auth CLI commands for generating and migrating database schemas, creating initial admins, initializing projects, generating secret keys, and gathering diagnostic info.
...
- [Upgrading to Better Auth 1.7](/llms.txt/docs/guides/1-7-upgrade-guide.md): Upgrade Better Auth from 1.6 to 1.7, including Expo, OAuth, OpenID Connect, MCP, SAML, SCIM, proxy, and custom adapter changes.
...
- [Client ID Metadata Document (CIMD)](/llms.txt/docs/plugins/cimd.md): Discover OAuth clients from HTTPS metadata documents.
...
- [MCP](/llms.txt/docs/plugins/mcp.md): Turn your Better Auth server into an OAuth provider for MCP clients
...
- [Groups and custom roles](/llms.txt/docs/plugins/scim/groups-and-roles.md): Provision SCIM Groups and map them to application roles.
- [System for Cross-domain Identity Management (SCIM)](/llms.txt/docs/plugins/scim.md): Provision users and groups from a directory into Better Auth.
- [SCIM reference](/llms.txt/docs/plugins/scim/reference.md): SCIM options, endpoints, resource behavior, and database schema.
...
- [state_invalid](/llms.txt/docs/reference/errors/state_invalid.md): Failed to decrypt or parse the OAuth state during the callback.
